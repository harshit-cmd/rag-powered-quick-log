...
const initVectorDb = async () => {
  const store = new Corestore("../../.rag-hyperdb");
  ...
  for (const sample of seedData1000plus) {
    sampleMap.set(sample.prompt, sample);
    samples.push(sample.prompt);
  }

  console.log("📚 Embedding documents...");
  const docs = await rag.saveEmbeddings(samples, {
      chunk: false,
  });
...
const performVectorSearch = async (query) => {
  ...
  return results.map(result => ({ ...sampleMap.get(result.content), score: result.score }));
};
